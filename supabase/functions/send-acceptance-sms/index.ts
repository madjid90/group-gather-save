...
interface AcceptanceSmsRequest {
  userId: string;
  telephone?: string;
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { userId, telephone }: AcceptanceSmsRequest = await req.json();

    console.log(`Sending acceptance SMS for user ${userId}`);
    ...
    const smsBody = `Merci ! Votre offre a été validée. Un conseiller prendra le relais si nécessaire.`;
    ...
    console.log("Acceptance SMS sent successfully:", twilioResult.sid);
    ...
    console.error("Error in send-acceptance-sms function:", error);
    return new Response(
