...
import { ArrowRight, Home, Building2, Flame, Zap, ChevronLeft, Calculator, TrendingDown, Sparkles, Snowflake, Droplets } from "lucide-react";
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="h-3 bg-muted rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 0.8, delay: 0.2 }} className="h-full bg-destructive/60 rounded-full" />
          </div>
        </div>
        <div className="space-y-2">
          <div className="flex justify-between items-center text-sm">
          ...
          <div className="h-3 bg-muted rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${(afterBill / maxBill) * 100}%` }} transition={{ duration: 0.8, delay: 0.4 }} className="h-full bg-primary rounded-full" />
          </div>
        </div>
      </div>
...
const ResultView = memo(function ResultView({
  ...
  return (
    <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center">
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: "spring", stiffness: 200 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-medium mb-4">
        <Sparkles className="w-4 h-4" />
        Félicitations ! Voici vos économies potentielles
      </motion.div>
      <h3 className="text-xl md:text-2xl font-bold text-foreground mb-6">Votre estimation d'économies</h3>
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }} className="bg-gradient-hero rounded-2xl p-6 md:p-8 mb-4">
        <p className="text-primary-foreground/80 text-sm mb-2">Économies estimées par an</p>
        <div className="flex items-baseline justify-center gap-2">
          <span className="text-4xl md:text-5xl font-bold text-primary-foreground">{estimation.minEconomie}€</span>
          <span className="text-2xl text-primary-foreground/70">à</span>
          <span className="text-4xl md:text-5xl font-bold text-primary-foreground">{estimation.maxEconomie}€</span>
        </div>
      </motion.div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mb-6">
        <Button variant="hero" size="lg" className="w-full md:w-auto px-8 shadow-lg" onClick={onCTAClick}>
          <Sparkles className="w-5 h-5 mr-2" />
          Voir les offres disponibles →
          <ArrowRight className="w-5 h-5 ml-2" />
        </Button>
        <p className="text-sm text-muted-foreground mt-2">Gratuit • Sans engagement • 30 secondes</p>
      </motion.div>
      <ComparisonChart profile={profile} savings={(estimation.minEconomie + estimation.maxEconomie) / 2} />
      <p className="text-muted-foreground text-sm md:text-base mb-4">{estimation.explication}</p>
      <p className="text-xs text-muted-foreground/70 mb-6">Estimation indicative basée sur des profils comparables. Aucune action sans votre accord.</p>
      <Button variant="outline" size="lg" className="w-full md:w-auto px-8 mb-4" onClick={onCTAClick}>
        Recevoir mon offre personnalisée <ArrowRight className="w-5 h-5 ml-2" />
      </Button>
      <button onClick={onReset} className="block mx-auto text-sm text-muted-foreground hover:text-foreground transition-colors underline">Refaire le calcul</button>
    </motion.div>
  );
});
...
export const SavingsCalculator = memo(function SavingsCalculator() {
  ...
  const [profile, setProfile] = useState<CalculatorProfile>({ logement: null, surface: null, chauffage: null, facture: null });
  ...
  const handleSelect = async (value: any) => {
    ...
    switch (step) {
      case 1: newProfile.logement = value; setProfile(newProfile); setStep(2); break;
      case 2: newProfile.surface = value; setProfile(newProfile); setStep(3); break;
      case 3: newProfile.chauffage = value; setProfile(newProfile); setStep(4); break;
      case 4:
  ...
  const getFallbackEstimation = (p: CalculatorProfile): EstimationResult => {
    ...
    else if (p.chauffage === "pompe_chaleur") { min += 40; max += 70; }
    else if (p.chauffage === "fioul") { min += 100; max += 180; }
    else if (p.chauffage === "collectif") { min -= 30; max -= 20; }
    ...
    const chauffageLabel = p.chauffage === "electrique" ? "l'électricité" :
      p.chauffage === "gaz" ? "l'électricité et le gaz" :
      p.chauffage === "pompe_chaleur" ? "l'électricité (pompe à chaleur)" :
      p.chauffage === "fioul" ? "l'énergie (fioul)" :
      p.chauffage === "collectif" ? "vos charges de chauffage collectif" : "l'énergie";

    return {
      minEconomie: Math.round(min / 10) * 10,
      maxEconomie: Math.round(max / 10) * 10,
      explication: `Pour ${logementLabel} avec votre profil, vous pouvez économiser sur ${chauffageLabel} en changeant de fournisseur via Switchly. Cette estimation est basée sur les économies réalisées par des foyers similaires.`
    };
  };
  ...
  const handleCTAClick = () => {
    ...
    const profileSummary = {
      ...
      chauffage: profile.chauffage === "electrique" ? "Électrique" : profile.chauffage === "gaz" ? "Gaz" : profile.chauffage === "pompe_chaleur" ? "Pompe à chaleur" : profile.chauffage === "fioul" ? "Fioul" : profile.chauffage === "collectif" ? "Collectif" : "Autre",
  ...
  const goBack = () => { if (typeof step === "number" && step > 1) setStep((step - 1) as Step); };
      ...
      <div className="container mx-auto px-4 md:px-6 w-full">
        <div className="max-w-2xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-5">
            ...
            <h2 className="text-xl md:text-3xl font-bold text-foreground mb-2">Combien pouvez-vous économiser ?</h2>
            <p className="text-sm text-muted-foreground">Répondez à quelques questions pour découvrir votre potentiel d'économies</p>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="bg-card border border-border rounded-xl md:rounded-3xl shadow-switchly-lg overflow-hidden">
            {step !== "result" && (
              <div className="px-4 pt-4">
                <div className="flex items-center justify-between text-sm text-muted-foreground mb-2">
                  <span>Étape {currentStep}/{totalSteps}</span>
                  {currentStep > 1 && (
                    <button onClick={goBack} className="flex items-center gap-1 hover:text-foreground transition-colors">
                      <ChevronLeft className="w-4 h-4" /> Retour
                    </button>
                  )}
                </div>
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <motion.div className="h-full bg-gradient-hero rounded-full" initial={{ width: 0 }} animate={{ width: `${(currentStep / totalSteps) * 100}%` }} transition={{ duration: 0.3 }} />
                </div>
              </div>
            )}
            ...
            <div className="p-4 md:p-8">
              <AnimatePresence mode="wait">
                {isLoading ? (
                  ...
                  <ResultView estimation={estimation} profile={profile} onCTAClick={handleCTAClick} onReset={resetCalculator} />
                ) : (
                      ...
                      {stepOptions[step as keyof typeof stepOptions].map((option) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-sm md:text-lg font-medium text-foreground flex-1">{option.label}</span>
