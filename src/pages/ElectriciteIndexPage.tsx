...
export default function ElectriciteIndexPage() {
  ...
  const departments = [...new Set(villes.map(v => v.departement).filter(Boolean))] as string[];
        ...
        <main className="flex-1 pt-20">
          <section className="py-10 bg-gradient-to-br from-primary/10 via-background to-secondary/5">
            <div className="container mx-auto px-4 max-w-3xl text-center">
              ...
              <h1 className="text-2xl md:text-4xl font-bold mb-3">Comparateur électricité par ville</h1>
              <p className="text-muted-foreground mb-6">Trouvez le fournisseur d'électricité le moins cher dans votre commune. Gratuit, sans engagement.</p>
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {villes.map(v => (
                      <Link key={v.slug} to={`/electricite/${v.slug}`} className="bg-card border border-border rounded-xl p-3 hover:border-primary transition-colors group">
