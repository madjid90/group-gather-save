...
// ── Électricité Index ────────────────────────────────────────
export default function ElectriciteIndexPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Comparateur électricité par ville — Switchly</title>
      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4 max-w-2xl text-center">
          ...
          <h1 className="text-2xl sm:text-4xl font-bold mb-3">
            Comparateur électricité par ville
          </h1>
          <p className="text-base text-muted-foreground mb-6">
            Trouvez le fournisseur d'électricité le moins cher dans votre commune. Gratuit, sans engagement.
          </p>
          <Button size="lg" className="bg-secondary hover:bg-secondary/90 text-white font-bold h-12 px-8" asChild>
            <Link to="/comparer?type=electricite">Comparer mon électricité →</Link>
          </Button>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full bg-card border border-border rounded-xl pl-9 pr-4 py-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
                {filtered.map(v => (
                  ...
                  >
                    <p className="font-semibold text-sm group-hover:text-primary transition-colors leading-tight">{v.nom}</p>
              ...
              {filtered.length === 0 && (
                <p className="text-center text-muted-foreground py-8">
                  ...
                  <Link to="/comparer" className="text-primary hover:underline text-sm font-medium mt-1 inline-block">
          ...
          <div className="mt-8 text-center">
            <Link to="/gaz/" className="text-primary hover:underline text-sm font-medium flex items-center justify-center gap-1">
              <Flame className="w-4 h-4" /> Voir aussi : Comparateur gaz par ville →
            </Link>
          </div>
        </div>
      </section>
