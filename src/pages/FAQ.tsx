...
const faqItems = [
  ...
  },
  {
    ...
    answer: "Oui. Propriétaire ou locataire, vous êtes libre de choisir votre fournisseur d'énergie. En tant que locataire, vous êtes généralement titulaire du contrat et pouvez donc le modifier librement. Aucune autorisation du propriétaire n'est nécessaire.",
  },
  {
...
export default function FAQ() {
  ...
  return (
    <PageTransition className="min-h-screen py-10 md:py-20 bg-gradient-subtle">
      <DynamicSEOHead 
        defaultTitle="FAQ - Switchly | Questions fréquentes sur le comparateur énergie"
        defaultDescription="Toutes vos questions sur Switchly, comparateur gratuit d'électricité et de gaz. Fonctionnement, économies, fournisseurs partenaires."
      />
      <div className="container mx-auto px-5 sm:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-8 lg:mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary mb-4">
            <HelpCircle className="w-4 h-4" />
            <span className="text-sm font-medium">Questions fréquentes</span>
          </div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground mb-4">Questions fréquentes</h1>
          <p className="text-base lg:text-lg text-muted-foreground max-w-2xl mx-auto">Tout ce que vous devez savoir sur Switchly</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="max-w-3xl mx-auto">
          <div className="bg-card rounded-2xl p-6 md:p-8 border border-border shadow-switchly-lg">
            <Accordion type="single" collapsible className="space-y-3">
              {faqItems.map((item, index) => (
                <AccordionItem key={index} value={`item-${index}`} className="border border-border rounded-xl px-4 data-[state=open]:bg-muted/50 transition-colors">
                  <AccordionTrigger className="text-left text-sm md:text-base font-medium hover:no-underline py-4 text-foreground">{item.question}</AccordionTrigger>
                  <AccordionContent className="text-sm md:text-base text-muted-foreground pb-4 leading-relaxed">{item.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-6 md:mt-12 text-center">
            <p className="text-sm text-muted-foreground mb-4">Vous avez d'autres questions ?</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button variant="outline" size="lg" className="py-3 text-sm" asChild>
                <Link to="/contact">Nous contacter</Link>
              </Button>
              <Button variant="hero" size="lg" className="py-3 text-sm" asChild>
                <Link to="/comparer">Comparer gratuitement → <ArrowRight className="w-5 h-5 ml-2" /></Link>
              </Button>
            </div>
          </motion.div>
        </motion.div>
